...
public interface PaperRepository extends GraphRepository<Paper> {
    ...
    @Query("MATCH (p:paper) WHERE p.pTitle = {paperTitle} " +
            ...
            "{field} " +
            " RETURN {r} LIMIT {k}")
    List<Object> findAllByAll(@Param("conference")String conference, @Param("author")String author, @Param("field")String field, @Param("publishYear")String publishYear, @Param("paperTitle")String paperTitle, @Param("r")String r,@Param("k")Integer limit);
